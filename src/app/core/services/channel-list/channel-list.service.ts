...
/**
 * Service for managing channel list display
 */
@Injectable({
  providedIn: 'root',
})
export class ChannelListService {
  ...
  /**
   * Get visible channels for current user (public + member channels)
   */
  ...
  /**
   * Map channel to list item with unread badges
   */
  ...
  private calculateNormalUnread(channel: any, messages: any[], isMember: boolean): boolean {
    ...
    const latestNormalMessageTime = this.getLatestNormalMessageTime(messages);
    const normalTimestamp = latestNormalMessageTime || this.getFallbackTimestamp(channel, messages);

    return normalTimestamp ? this.unreadService.hasUnread(channel.id, normalTimestamp) : false;
  }
  ...
  /**
   * Get fallback timestamp from channel data
   */
